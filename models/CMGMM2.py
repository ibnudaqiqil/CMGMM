...
class CMGMM2(GaussianMixture):
	'''
	classdocs
	'''

	def __init__(self,min_components=3, max_step_components=30, max_components=60,distance_method="Kullback-leiber",merge_type="moment_presaving"):

		GaussianMixture.__init__(self, n_components=min_components,covariance_type='full')
	...
	def fit(self, data):
		...
		if (self.initialized==False):			
			...
			w_all = np.concatenate((self.backup_weights_ , best_gmm.weights_),axis=None)
			#w_all = w_all/np.sum(w_all)
			mu_all =  np.concatenate((self.backup_means_ , best_gmm.means_ ),axis=0)
			cov_all = np.concatenate((self.backup_covariances_ , best_gmm.covariances_ ),axis=0)

			#n_comp =5
			n_components_range =  range(self.max_components+1, self.min_components, -1)
			bicreduced=[]
			lowest_bic = np.infty
			...
			self.precisions_cholesky_ = _compute_precision_cholesky(best_gmm.covariances_, self.covariance_type)
