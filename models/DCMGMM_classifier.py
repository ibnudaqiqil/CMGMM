...
class DCMGMMClassifier(BaseNeighbors, ClassifierMixin):
    model = defaultdict()
    ...
    def __init__(self,
        ...
        for scene_label in self.classes:
            self.model[scene_label] = CMGMM(
                min_components=4, max_components=20, pruneComponent=prune_component)
            if(drift_detector != None):

                self.drift_detector[scene_label] = copy.deepcopy(
                    drift_detector)
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        if (self.drift_detector == None):
            ...
            for dt in X:
                ...
                if(isDetected):
                    print("adaptation:", label, "=>",
                          i, len(self.driftData[label]))
                    self.adaptasi += 1
